...
const init = async (client) => {
    try {
        ...
        if (userbalance.USDC.available > 5) {
            ...
            const sellPercentage = Math.random() * (0.9 - 0.6) + 0.6; // 生成随机卖出比例
            await sellfun(client, sellPercentage);
            return;
        }
        // 添加随机等待时间，范围为5到15秒
        const randomWaitTime = Math.floor(Math.random() * 61) + 30;
        console.log(getNowFormatDate(), `等待 ${randomWaitTime} 秒后进行下一笔交易...0`);
        await delay(randomWaitTime * 1000); // 将秒转换为毫秒
    } catch (e) {
...
const sellfun = async (client, sellPercentage) => {
    //取消所有未完成订单
    ...
    // 计算卖出数量，使用传递的随机卖出比例
    let sellQuantity = parseFloat((userbalance2.SOL.available * sellPercentage).toFixed(2));
    console.log(getNowFormatDate(), `正在卖出中... 卖${sellQuantity}个SOL`);
    ...
    if (orderResultAsk?.status == "Filled" && orderResultAsk?.side == "Ask") {
        ...
        // 添加随机等待时间，范围为5到15秒
        const randomWaitTime = Math.floor(Math.random() * 301) + 100;
        console.log(getNowFormatDate(), `等待 ${randomWaitTime} 秒后进行下一笔交易...1`);
        await delay(randomWaitTime * 1000); // 将秒转换为毫秒
        init(client); // 重新开始新一轮交易
    } else {
